...
pub fn main() -> Result<()> {
    ...
    let effective_assembers = config.effective_assembers();
    for assembler in effective_assembers {
        assembler.setup_directives(&mut names_to_info.directives);
    }

    let effective_archs = config.effective_archs();
    for instruction_set_arch in effective_archs {
        instruction_set_arch.setup_instructions(&mut names_to_info.instructions);
        instruction_set_arch.setup_registers(&mut names_to_info.registers);
    }
